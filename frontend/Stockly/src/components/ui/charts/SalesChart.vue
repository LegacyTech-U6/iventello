  ...
  <div class="w-full bg-white p-3 rounded-xl border border-gray-100 shadow-sm transition-all duration-300">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
      <div>
        <h2 class="text-xl font-bold text-gray-800 flex items-center gap-2">
          <div class="p-2 bg-gray-50 rounded-lg">
            <ShoppingCartIcon class="w-5 h-5 text-gray-700" />
          </div>
          Ventes & Achats
        </h2>
      ...
      <div class="flex bg-gray-100 p-1 rounded-xl self-start border border-gray-200">
        <button v-for="tab in tabs" :key="tab.value" @click="activeTab = tab.value" :disabled="isLoading"
    ...
    <div class="relative w-full overflow-hidden" :style="{ height: chartHeight + 60 + 'px' }">
      ...
      <div v-if="isLoading"
        class="absolute inset-0 z-20 flex flex-col items-center justify-center bg-white/60 backdrop-blur-[1px] transition-all">
      ...
      <svg :viewBox="`0 0 ${chartWidth} ${chartHeight + 40}`"
        ...
        <g v-for="(item, i) in chartData" :key="`group-${i}`">
          <rect :x="getX(i)" :y="getY(item.purchase)" :width="barWidth" :height="getH(item.purchase)" fill="#ef4444"
            rx="4" class="cursor-pointer hover:fill-red-600 transition-all duration-300 opacity-80 hover:opacity-100"

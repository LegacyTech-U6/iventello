<template>
    <div
        class="bg-white border border-gray-200 rounded-xl shadow-sm p-5 space-y-3 h-full flex flex-col justify-between transition-all hover:shadow-lg">
        <div class="flex items-center justify-between">
            <span class="text-md font-medium text-gray-400 capitalize">{{ label }}</span>
            <component :is="icon" class="w-6 h-6 text-blue-500" v-if="typeof icon !== 'string'" />
            <span class="material-symbols-outlined text-xl text-blue-500" v-else>{{ icon }}</span>
        </div>

        <div v-if="isEditing" class="flex items-center gap-2">
            <input :value="modelValue" @input="onInput" :type="isCurrency ? 'number' : 'number'" :min="0"

<template>
  <Teleport to="body">
    <div
      class="absolute inset-0 w-full h-full z-50 backdrop-blur-sm overflow-y-auto flex justify-center py-10"
    ...
    >
      ...
      <div class="absolute top-4 left-0 w-full px-6 flex justify-between items-start z-[60] pointer-events-none">
        ...
        <div
          class="pointer-events-auto flex items-center gap-2 backdrop-blur-md p-2 rounded-lg border shadow-xl"
          ...
          <button @click="downloadPDF"
            class="text-white px-4 py-2 rounded-md font-bold flex items-center gap-2 transition-colors text-sm shadow-lg"
            style="background-color: #059669;"> <ArrowDownTrayIcon class="h-4 w-4" />
            PDF
          </button>
        </div>
      </div>
  ...
  style="background-color: #ffffff; color: #111827;">
        ...
        <div class="border-b-2 pb-6 mb-8" style="border-color: #1f2937;"> <div class="flex justify-between items-start">
            <div>
              <h1 class="text-4xl font-extrabold tracking-wide uppercase" style="color: #111827;">Invoice</h1>
              <div class="mt-4 text-sm" style="color: #4b5563;"> <CompanyInfo :entreprise="entreprise" />
              </div>
            </div>
            <div class="text-right">
              <div class="p-4 rounded-lg border" style="background-color: #f9fafb; border-color: #f3f4f6;"> <div class="mb-2">
                  <span class="text-xs uppercase font-bold tracking-wider" style="color: #6b7280;">Invoice #</span> <div class="font-bold text-xl" style="color: #111827;">{{ invoice.id }}</div>
                </div>
        ...
        <div class="mb-8 grid grid-cols-2 gap-8">
          <div>
            <h3 class="text-xs font-bold uppercase tracking-wider mb-2" style="color: #9ca3af;">Billed To</h3> <div class="font-bold text-lg" style="color: #111827;">
        ...
        <div class="mb-8">
          <table class="w-full text-sm">
            <thead>
              <tr class="border-y" style="background-color: #f9fafb; border-color: #e5e7eb;"> <th class="py-3 pl-4 text-left font-semibold uppercase text-xs tracking-wider" style="color: #4b5563;">#</th>
                <th class="py-3 text-left font-semibold uppercase text-xs tracking-wider w-1/2" style="color: #4b5563;">Description</th>
                <th class="py-3 text-center font-semibold uppercase text-xs tracking-wider" style="color: #4b5563;">Qty</th>
                <th class="py-3 text-right font-semibold uppercase text-xs tracking-wider" style="color: #4b5563;">Price</th>
                <th class="py-3 pr-4 text-right font-semibold uppercase text-xs tracking-wider" style="color: #4b5563;">Total</th>
              </tr>
            </thead>
            <tbody class="divide-y" style="border-color: #f3f4f6;"> <tr v-for="(item, index) in invoice.items" :key="item.id">
                ...
                <td class="py-4 text-center font-mono" style="color: #374151;">{{ item.quantity }}</td> <td class="py-4 text-right font-mono" style="color: #374151;">{{ format(item.unit_price) }}</td>
          ...
          <div class="md:w-1/2 space-y-6">
            <div class="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p class="text-sm font-medium" style="color: #111827;">{{ invoice.mode_paiement || 'Not specified' }}</p>
              </div>
            </div>

            <div v-if="invoice.notes">
              <h4 class="text-xs font-bold uppercase tracking-wider mb-2" style="color: #9ca3af;">Notes</h4>
              <p class="text-sm italic p-3 rounded border" style="background-color: #f9fafb; border-color: #f3f4f6; color: #4b5563;">
                {{ invoice.notes }}
              </p>
            </div>
          </div>
          ...
          <div class="md:w-5/12">
            <div class="space-y-3">
              <div class="flex justify-between text-sm" style="color: #4b5563;">
                <span>Subtotal</span>
                <span class="font-mono font-medium" style="color: #111827;">{{ format(invoice.total_hors_reduction) }}</span>
              </div>
              <div class="border-t pt-3 mt-3 flex justify-between items-center" style="border-color: #e5e7eb;">
                ...
                <span class="text-xl font-bold font-mono" style="color: #059669;">{{ format(invoice.total) }}</span>
              </div>
            </div>
          </div>
        </div>
...
@media print {
  ...
  .bg-black\/85 { background: white !important; }
